...
イベント名の先頭に Bluesky のロゴアイコン (Font Awesome のブランドアイコン、
ブランドカラー `#0085ff`) を添えます。イベント名の文字列そのものは変わりません。
...
| `ALLOWED_LANGS` | 空 (全言語) | 取り込む投稿の言語 (カンマ区切り)。ウォールごとにさらに絞れる |
| `WALL_COLUMNS` | `1` | カラム数。`1` で縦に流れる。`2` 以上で横並びのグリッド |
| `WALL_MAX_CARDS` | `40` | DOM 上に保持する最大カード数。スクロールで遡れる範囲 |
| `SHOW_BLUESKY_LOGO` | `true` | イベント名の先頭に Bluesky のロゴアイコンを出すか (テナント単位) |
